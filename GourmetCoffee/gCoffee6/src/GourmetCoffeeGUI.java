...
public class GourmetCoffeeGUI extends JPanel {
	...
	private JPanel getDataFieldsPanel(ArrayList<DataField> dataFields) {
		...
		for (DataField data : dataFields) {
			label = new JLabel(data.getName() + " :");
			text = new JTextField(data.getValue());
	...
	class AddModifyListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent event) {
			...
			totalTextField.setText("$" + Double.parseDouble(new DecimalFormat("#.##").format(
	...
	class RemoveListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent event) {
			...
			if (currentOrder.getNumberOfItems() == 0) {
				statusTextArea.setText("There is no any order.");
			} else {
				...
				if (item != null) {
					...
					orderList.setListData(currentOrder.getItems());
					totalTextField.setText("$" + Double.parseDouble(
							new DecimalFormat("#.##").format(currentOrder.getTotalCost())));
			...
			if(result == JFileChooser.APPROVE_OPTION) {
				try{
					...
					PrintWriter writer = new PrintWriter(
							new FileWriter(file));
					String formatString = statusTextArea.getText();
