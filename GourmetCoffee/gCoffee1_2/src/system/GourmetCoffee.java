...
public class GourmetCoffee  {
	...
	public void displayNumberOfOrders(Product product) {
		...
		int size = sales.getNumberOfOrders();
		int num = 0;
		
		if (size == 0) {
			stdErr.println("There is no any order sold!");
		} else {
			for (Order orders : sales) {
				if (orders.getItem(product) != null)
					num++;
				}
			}
			
			stdOut.println("Number of orders that contains the product A001: " + num);
	}
	...
	public void displayTotalQuantityOfProducts() {
		...
		int size_1 = catalog.getNumberOfProducts();
		
		if (size_1 == 0) {
			stdOut.println("There is no product in the catalog!");
		} else {
			// find products in the catalog
			for (Product pro : catalog) {
				int quantity = 0;
				int size_2 = sales.getNumberOfOrders();
				
				if (size_2 == 0) {
					stdErr.println("There is no any order sold!");
				} else {
					// find orders in the sales
					for (Order orders : sales) {
						// find the orderitem which has the product above and calculate the quantity of it.
						for (OrderItem item : orders) {
							if (item.getProduct().equals( pro )) {
								quantity += item.getQuantity();
							}
							}
						}
					}
				stdOut.println(pro.getCode() + " " + quantity);
			}
		}
